...
export const askPoultryExpert = async (question: string, attachments: Attachment[] = [], language: string = 'English'): Promise<ExpertResponse> => {
  try {
    // Set to gemini-1.5-flash for better free-tier rate limits (1500 RPD)
    ...
    */
    ...
    // Check if error is a JSON string containing quota error
    try {
      if (typeof errorMsg === 'string' && errorMsg.includes('{')) {
         const parsed = JSON.parse(errorMsg.substring(errorMsg.indexOf('{')));
         if (parsed?.error?.code === 429 || parsed?.error?.status === 'RESOURCE_EXHAUSTED') {
            errorMsg = "API Request limit exceeded. Please wait a moment and try again.";
    ...
    return { 
      text: `Failed to connect to the Poultry Expert AI. Error: ${errorMsg}. Please try again later or check your network.`, 
      sources: [] 
    };
  }
};
...
export const generateFarmingImage = async (prompt: string): Promise<string | null> => {
  // Free tier API keys currently have 0 quota for Imagen models.
  // We use reliable Unsplash placeholders matching the context visually to avoid breaking the UI.
  try {
     const lowerPrompt = prompt.toLowerCase();
     if (lowerPrompt.includes("setup")) {
         return "https://images.unsplash.com/photo-1548550023-2bf3c49bceee?auto=format&fit=crop&q=80&w=800";
     } else if (lowerPrompt.includes("temp") || lowerPrompt.includes("behavior")) {
         return "https://images.unsplash.com/photo-1560743641-3914f2c45636?auto=format&fit=crop&q=80&w=800";
     } else if (lowerPrompt.includes("hen") || lowerPrompt.includes("incubation")) {
         return "https://images.unsplash.com/photo-1614031679247-975ba6646536?auto=format&fit=crop&q=80&w=800";
     } else if (lowerPrompt.includes("cage") || lowerPrompt.includes("housing")) {
         return "https://images.unsplash.com/photo-1516467508483-a7212febe31a?auto=format&fit=crop&q=80&w=800";
     }
     return "https://images.unsplash.com/photo-1518556137688-693282eb1113?auto=format&fit=crop&q=80&w=800";
  } catch (error) {
...
export const getWeatherAndBroodingAdvice = async (location: string): Promise<WeatherAdvice | null> => {
    try {
        ...
        // Fallback for free-tier users or network issues
        return {
            location: location,
            temp: "28°C",
            humidity: "55%",
            condition: "Clear Sky",
            broodingTip: "Maintain steady temperature at 95°F for Week 1. Ensure no drafts in the brooding area.",
            generalTip: "Keep waterers clean and provide high-quality pre-starter feed."
        };
    }
};
