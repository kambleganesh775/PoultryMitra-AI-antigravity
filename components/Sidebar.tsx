...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const handleBackup = async () => {
      try {
          const dataStr = await dataService.exportDatabase();
          const targetStr = `data:application/json;charset=utf-8,${encodeURIComponent(dataStr)}`;
          const link = document.createElement('a');
          link.href = targetStr;
          link.download = `PoultryMitra_Backup_${new Date().toISOString().split('T')[0]}.json`;
